...
export const AddYeartoCourse = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const GetYearsOfCourse = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const years = course.years.map((year) => ({
      _id: year._id,
      year: year.year,
    }));
...
export const EditYear = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const DeleteYear = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { courseId, yearId } = req.params;
      ...
      const yearToDelete = course.years.find(
        (y) => y._id.toString() === yearId
      );
      if (!yearToDelete) {
        return res
          .status(404)
...
export const AddSubjectToYear = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { courseId, yearId } = req.params;
      const { name } = req.body;
      ...
      const SubjectModel = mongoose.model(
        "Subject",
        CourseModel.schema.paths.years.schema.paths.subjects.schema
      );
...
export const EditSubject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const subjectToEdit = year.subjects.find(
...
export const DeleteSubject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const subjectIndex = year.subjects.findIndex(
        (s) => s._id.toString() === subjectId
      );
      if (subjectIndex === -1) {
        return res
    ...
    try {
      ...
      if (!courseId || !yearId) {
        return res
          ...
          .json({
            success: false,
            message: "Course ID and Year ID are required",
          });
      }
...
export const AddQuestToSubject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { courseId, yearId, subjectId } = req.params;
      ...
      if (questionImage) {
        const result = await cloudinary.v2.uploader.upload(
          (questionImage as any).tempFilePath,
          {
            folder: "questions",
          }
        );
      ...
      if (answerImage) {
        const result = await cloudinary.v2.uploader.upload(
          (answerImage as any).tempFilePath,
          {
            folder: "answers",
          }
        );
      ...
      const year = (course.years as IYear[]).find(
        (y) => y._id.toString() === yearId
      );
      if (!year) {
        return res
      ...
      const subject = (year.subjects as ISubject[]).find(
...
export const UpdateQuestInSubject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const DeleteQuestFromSubject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { courseId, yearId, subjectId, questionId } = req.params;
...
// export const LikeQuestion = CatchAsyncError(
//   async (req: Request, res: Response, next: NextFunction) => {
//     const { courseId, yearId, subjectId, questionId } = req.params;
...
// export const DislikeQuestion = CatchAsyncError(
//   async (req: Request, res: Response, next: NextFunction) => {
//     const { courseId, yearId, subjectId, questionId } = req.params;
...
//       return res.status(400).json({
//         success: false,
//         message: "You have already disliked this question",
//       });
//     }
...
export const UnlikeQuestion = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Remove user from likedBy array and decrease likes count
...
export const getTotalLikesAndDislikes = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    const { courseId } = req.params;
    ...
    const allQuestions = course.years.flatMap((year) =>
      year.subjects.flatMap((subject) => subject.questions)
    );

    const totalLikes = allQuestions.reduce(
      (total, question) => total + (question.likes ?? 0),
      0
    );
    const totalDislikes = allQuestions.reduce(
      (total, question) => total + (question.dislikes ?? 0),
      0
    );
...
export const getUserLikeDislikeDetails = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
            ...
            $cond: {
              if: {
                $in: [
                  ...
                  "$years.subjects.questions.likedBy",
                ],
              },
              then: true,
              else: false,
          ...
          dislikedBy: {
            $cond: {
              if: {
                $in: [
                  new Types.ObjectId(userId),
                  "$years.subjects.questions.dislikedBy",
                ],
              },
              then: true,
              else: false,
...
export const getQuestionLikeDislikeDetails = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    const { courseId, yearId, subjectId, questionId } = req.params;
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!Array.isArray(questions) || questions.length === 0) {
        return next(
          new ErrorHandler("Questions must be a non-empty array.", 400)
        );
      }
      ...
      const validatedQuestions = questions.map((question) => {
        if (
          ...
          throw new ErrorHandler(
            "Invalid question format. Each question must have a valid questionNumber.",
            400
          );
        }
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!doubts || doubts.length === 0) {
        return res
          .status(404)
          .json({ message: "No doubts found for this user." });
      }
      ...
      const doubtsWithCourseInfo = await Promise.all(
        doubts.map(async (doubt: IDoubt) => {
          ...
          const course = await CourseModel.findOne({
            "years.subjects.questions.questionText": {
              $regex: new RegExp(`^(${questionNumbers.join("|")})q$`),
            },
          });
          ...
          if (!course) {
            console.log(
              "No course found for question numbers:",
              questionNumbers
            );
          ...
          const relevantQuestions: IQuestion[] = relevantSubjects.flatMap(
            (subject) =>
          ...
          const courseInfo = {
            ...
            years: relevantYears.map((year) => ({
              ...
              subjects: year.subjects
                .filter((subject) =>
                  subject.questions.some((q) =>
                    questionNumbers.includes(parseInt(q.questionText))
                  )
                )
                .map((subject) => ({
                  _id: subject._id,
